describe ExchangeProviders::Cnb::RatesProvider do
  ...
  let(:rates_provider) { described_class.new(source_currency: source_currency, target_currency: target_currency) }
  ...
  describe "#fixed_rates" do
    ...
    before do
      allow(ExchangeProviders::Cnb::FixedRatesProvider).to(
        ...
        ).and_return(fixed_rates_provider)
      )
      allow(fixed_rates_provider).to receive(:call).and_return(expected_rates)
    end

    it do
      expect(ExchangeProviders::Cnb::FixedRatesProvider).to(
        receive(:new).with(
          source_currency: source_currency,
          target_currency: target_currency
        ).once
